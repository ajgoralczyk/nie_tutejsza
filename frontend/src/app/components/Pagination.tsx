...
export default function Pagination(props: PaginationProps) {
  ...
  return (
    <div className="pb-4 xl:pb-6 flex gap-2 justify-center">
      ...
      <PaginationButton url={createPageURL(1)} isCurrent={currentPage === 1}>
        1
      </PaginationButton>
      {pageCount > 2 && (
        <>
          {currentPage - 2 > 2 && (
            <PaginationButton isDisabled={true}>...</PaginationButton>
          )}
          {currentPage - 2 > 1 && (
            <PaginationButton url={createPageURL(currentPage - 2)}>
              {currentPage - 2}
            </PaginationButton>
          )}
          {currentPage - 1 > 1 && (
            <PaginationButton url={createPageURL(currentPage - 1)}>
              {currentPage - 1}
            </PaginationButton>
          )}
          {currentPage > 1 && (
            <PaginationButton isCurrent={true}>{currentPage}</PaginationButton>
          )}
